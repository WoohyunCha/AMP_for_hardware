...
class AMPLoader:
    FRAME_TIME = 0
    OBSERVATIONS_START = 1
    NUM_ACTIONS = 12

    JOINT_POSE_START_IDX = 0
    JOINT_POSE_END_IDX = NUM_ACTIONS
    JOINT_VEL_START_IDX = JOINT_POSE_END_IDX
    JOINT_VEL_END_IDX = JOINT_VEL_START_IDX + NUM_ACTIONS
    ...
    def __init__(
        ...
        for i, motion_file in enumerate(motion_files):
            ...
            with open(motion_file, "r") as f:
                ...
                motion_data = expand_data(motion_data, motion_data[1,AMPLoader.FRAME_TIME] - motion_data[0,AMPLoader.FRAME_TIME])
                ...
                self.trajectories.append(torch.tensor( # trajectory = [dof_pos, dof_vel]
                    expand_data(motion_data[:, AMPLoader.OBSERVATIONS_START:AMPLoader.OBSERVATIONS_START+AMPLoader.NUM_ACTIONS], motion_data[1, AMPLoader.FRAME_TIME] - motion_data[0, AMPLoader.FRAME_TIME])
    ...
    def slerp(self, val0, val1, blend): # interpolation between val0 and val1 with weights blend
    ...
    def get_full_frame_at_time(self, traj_idx, time): # Used when amp observations include task space obs
        ...
        # blend = p * n - idx_low
        return self.get_frame_at_time(traj_idx=traj_idx, time=time)
    ...
    def observation_dim(self):
        ...
        return self.trajectories[0].shape[1] # Without time stamp
...
def expand_data(traj: np.ndarray, dt: float): # Euler approximation for dof velocity
    traj_prev = np.zeros_like(traj)
    traj_prev[1:] = traj[:-1]
    traj_prev[0] = traj[0]
    return np.concatenate((traj, (traj-traj_prev)/dt), axis=-1)
